  ...
  constructor(
    ...
    ngOnInit() {
        ...
        if (this.idItem) {
            this.itemService.getItemById(this.idItem)
                ...
                .catch(error => {
                    this.messageService.add({severity: 'error', summary: 'Error', detail: 'No fui posible efectuar la operacion'});
                });
        }
        this.itemForm = this.fb.group({
            'id': null,
            '_about': new FormControl(null),
    ...
    onSubmit(value: string) {
        this.itemService.save(value).then(_ => {
           // this.itemForm.reset();
            this.messageService.add({severity: 'info', summary: 'Exito', detail: 'Operacion efectuado con exito!!'});
        }).catch( erro => {
            this.messageService.add({severity: 'error', summary: 'Error', detail: 'No fue posible efectuar la operacion'});
        });
    }
    formInvalid() {
